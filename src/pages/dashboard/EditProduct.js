...
import { useEditProductMutation, useGetSingleProductQuery } from '../../features/product/productApi';
...
const EditProduct = () => {
    ...
    // get the product data if product is exist
    const { data: product, isError } = useGetSingleProductQuery(slug);
    ...
    const EditProductHandler = (e) => {
        ...
        if (descriptionRef.current) {
            ...
            editProduct({slug, body: formData});
    ...
    return (
        <DashboardWrapper title="Edit Product | Dashboard">
            <div className='m-5'>
                <div className='py-5'>
                    <h3 className='text-xl font-medium uppercase text-gray-800'>Edit Product</h3>
                </div>

                <form id="productForm" onSubmit={EditProductHandler}>
                        ...
                        <div className='lg:col-span-4 col-span-12 p-3'>
                                ...
                                <select className='input-box mt-2' value={category} onChange={e => setCategory(e.target.value)} required>
                                    {
                                        categories?.map((cat, key) => (
                                            <option key={key} value={cat._id} >{cat.name}</option>
                                        ))
                                    }
                                </select>
                            </div>
                                    ...
                                    image && <div className="relative">
                                        ...
                                        </div>
                                        {
                                            image?.name && image?.size ? <img
                                                ...
                                                alt=""
                                            /> : <Image url={image} className='w-full h-72 object-cover' />
                                        }
                            ...
                            <div className='mt-5'>
                                <button type='submit' disabled={isLoading} className='submit-btn flex justify-center gap-3 capitalize w-full'>
                                    {
                                        isLoading ? <BiLoader className='animate-spin' /> : <span>Save Product</span>
                                    }
                                </button>
                            </div>
